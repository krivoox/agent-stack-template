...
const SIDEBAR_WIDTH_MOBILE = "min(100vw - 1rem, 20rem)"
...
function SidebarProvider({
  ...
  // Close the sheet when the route changes — adjust during render, not in an
  // effect, so we do not schedule a cascading update after paint.
...
function SidebarMenuSub({ className, ...props }: React.ComponentProps<"ul">) {
  return (
    <ul
      ...
      className={cn(
        "mx-3.5 flex min-w-0 flex-col gap-1 px-2.5 py-0.5 group-data-[collapsible=icon]:hidden",
        className
      )}
      {...props}
    />
